...
/**
 * Static import-analysis guard (FR-002, SC-008): `@whiskeysockets/baileys` may
 * only be imported inside `backend/src/whatsapp/`. Any import elsewhere —
 * backend, frontend, or shared — means library types are leaking past the
...
const LIBRARY_IMPORT = /(?:from\s+|require\(\s*|import\(\s*)['"]@whiskeysockets\/baileys['"]/;
...
describe('@whiskeysockets/baileys containment (SC-008)', () => {
  it('has no baileys imports outside backend/src/whatsapp/', async () => {
  ...
  it('keeps the real adapter inside the allowed directory (sanity check)', async () => {
    ...
    const adapter = await readFile(join(ALLOWED_DIR, 'BaileysAdapter.ts'), 'utf8');
