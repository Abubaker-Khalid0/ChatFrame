...
function renderProgress(state: unknown = { importId: 'imp_1', request }) {
  return render(
    <MemoryRouter initialEntries={[{ pathname: '/import/progress', state }]}>
      <Routes>
        <Route path="/import/progress" element={<ImportProgressPage />} />
        <Route path="/quality" element={<QualityProbe />} />
...
function QualityProbe() {
  ...
  return <div data-testid="quality-state">{JSON.stringify(location.state)}</div>;
}
...
describe('ImportProgressPage — live rendering (FR-019, FR-020, US3)', () => {
  it('subscribes to import events for the lifetime of the screen', () => {
  ...
  it('navigates to the quality screen on completion (FR-022)', async () => {
    renderProgress();
    applyProgress({ stage: 'completed', messagesImported: 12, messagesTotal: 12 });

    await waitFor(() => {
      const forwarded = JSON.parse(screen.getByTestId('quality-state').textContent ?? '{}') as {
        projectId?: string;
      };
      expect(forwarded.projectId).toBe('chatframe_2026-06-12_Layla');
    });
  });
});

describe('ImportProgressPage — cancellation (FR-021, US4)', () => {
  ...
  it('Back returns to the chat picker; Retry starts a fresh import (FR-030)', async () => {
    ...
    // The store was reset for the fresh attempt.
    expect(useImportStore.getState().progress).toBeNull();
  });
});

describe('ImportProgressPage — fatal errors (US5 AC-3)', () => {
  it('shows a clear, sanitized failure message with Back and Retry', () => {
    ...
    // No stack traces or internals leak into the view.
